...
ways to do standardization. We use the
...
- The product term, `iv:mod`, is also
  standardized (`iv_x_mod` in this model).
...
to disabled it because we only want to
...
#> Warning in summary.glm_betaselect(glm_beta_select): Bootstrap estimates not
#> available; 'se_method' changed to 'default'.
...
A warning is always issued if standardization
is done but bootstrapping not requested.
...
with the product
term computed after `iv` and `mod` are
standardized and bootstrap confidence
interval used, we can call `glm_betaselect()`
...
is much fewer than the variables
not to standardize.

- Use `not_to_standardize`
when the variables to standardize
...
variables except for `dv`, `mod`, we can use
this call, and set
`not_to_standardize` to `c("dv", "mod")`:
