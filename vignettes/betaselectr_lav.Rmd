...
in an interaction does not require that these
variables are mean-centered, since Version 0.1.2.2.
If a variable is standardized, and so
is also mean-centered, and it is involved
in an interaction (e.g., `x` in `x:w`),
its coefficient (e.g., `x`'s effect when
`w` is equal to zero) will be recomputed
accordingly.
