...
- The confidence intervals are formed using
  the ordinary least squares (OLS), which does not
...
The function `lm_beta_iv_mod()` can be
...
#> Warning in summary.lm_betaselect(lm_beta_select): Bootstrap estimates not
#> available; 'se_method' changed to 'ls'.
...
can be a severely biased estimate of the
...
with the product
term computed after `iv`, `mod`, and `dv` are
standardized and bootstrap confidence
interval used, we can call `lm_betaselect()`
